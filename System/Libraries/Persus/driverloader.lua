...
local Logger = require '/System/Libraries/Persus/logger'
...
function DriverLoader:Load()
    for index, value in ipairs(fs.list("/System/Drivers")) do
        _G.Persus.Logger:Msg("Persus::DriverLoader: Loading driver: " .. value)
        local driver = {
...
function DriverLoader:GetLoadedDriverByName(name)
    ...
    error("Persus::DriverLoader: Failed to find driver: " .. name)
end
function DriverLoader:Unload()
    _G.Persus.Logger:Msg("Persus::DriverLoader: Say goodnight, Gracie.")
    for index, value in ipairs(DriverLoader.Drivers) do
        value.Driver:Unload()
        table.remove(DriverLoader.Drivers, index)
    end
    _G.Persus.Logger:Msg("Persus::DriverLoader: Unloaded all Drivers.")
end
return DriverLoader