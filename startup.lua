...
_G._HOST = _G._HOST .. " (Andromeda UnBIOS)"

local function local_write(sText)
		...
		if text then
			...
			if #text > w then
				while #text > 0 do
					...
					local part = string.sub(text, 1, w - x + 1)
					term.write(part)
					text = string.sub(text, #part + 1)
					x, y = term.getCursorPos()
				end
			else
				if x + #text - 1 > w then
...
local function downWrite(msgType, text)
	local_write(string.format("[%-7s] %s", msgType:upper(), text))
...
function _G.term.native()
	...
	for _,v in ipairs(kernelArgs) do
		if v == "-v" or v == "--verbose" then
			isVerbose = true
			break
		end
	end
	if isVerbose then downWrite("info", "Loading the Cepheus kernel") end
	local file = fs.open("/System/Libraries/Kernels/cepheus.lua", "r")
	if file == nil then
		term.setCursorBlink(false)
		term.setTextColor(16384)
		downWrite("ERROR", "Could not find the Andromeda Cepheus")
		downWrite("ERROR", "kernel. UnBIOS cannot continue.")
		coroutine.yield("key")
		os.shutdown()
	end
	local fn, err = loadstring(file.readAll(), "@cepheus.lua")
	file.close()
	if fn == nil then
		term.setCursorBlink(false)
		term.setTextColor(16384)
		downWrite("ERROR", "Could not load the Andromeda Cepheus")
		downWrite("ERROR", "kernel. UnBIOS cannot continue.")
		downWrite("ERROR", err)
		coroutine.yield("key")
		os.shutdown()
	...
	os.shutdown = function()
		...
		return fn(table.unpack(kernelArgs))
	end
end
if debug then
